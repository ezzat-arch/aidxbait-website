"use client";

import { motion } from "framer-motion";
import TestimonialsCarousel from "@/components/ui/testimonials-carousel";
import type { Testimonial } from "@/components/ui/testimonials-carousel";

const testimonials: Testimonial[] = [
	{
		quote:
			"Doctoory made my post-surgery recovery so much easier. The home visit scheduling and guided exercises were a game changer!",
		name: "Sarah Johnson",
		designation: "Knee Replacement Patient",
		src: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=3560&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
	},
	{
		quote:
			"As a physical therapist, I love how Doctoory lets me track patient progress and adjust programs remotely. The analytics are fantastic!",
		name: "Michael Chen",
		designation: "Physical Therapist",
		src: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=3540&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
	},
	{
		quote:
			"Booking online consultations was seamless and the reminders kept me on track. I felt supported every step of my rehab journey.",
		name: "Emily Rodriguez",
		designation: "ACL Recovery Patient",
		src: "https://images.unsplash.com/photo-1623582854588-d60de57fa33f?q=80&w=3540&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
	},
	{
		quote:
			"The product store is so convenient! I found all the rehab equipment I needed and the delivery was fast.",
		name: "James Lee",
		designation: "Hip Surgery Patient",
		src: "https://images.unsplash.com/photo-1636041293178-808a6762ab39?q=80&w=3464&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
	},
	{
		quote:
			"Doctoory helps our clinic coordinate care and communicate with patients more efficiently. The platform is intuitive and reliable.",
		name: "Priya Patel",
		designation: "Orthopedic Nurse",
		src: "https://images.unsplash.com/photo-1624561172888-ac93c696e10c?q=80&w=2592&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
	},
];

export function TestimonialsSection() {
	return (
		<section id="testimonials" className="bg-white py-20">
			<motion.div
				initial={{ opacity: 0 }}
				whileInView={{ opacity: 1 }}
				viewport={{ once: true, amount: 0.2 }}
				transition={{ duration: 0.8 }}
			>
				<motion.div
					initial={{ opacity: 0, y: 40 }}
					whileInView={{ opacity: 1, y: 0 }}
					viewport={{ once: true, amount: 0.3 }}
					transition={{ duration: 0.6, delay: 0.2 }}
				>
					<TestimonialsCarousel testimonials={testimonials} autoplay />
				</motion.div>
			</motion.div>
		</section>
	);
}
