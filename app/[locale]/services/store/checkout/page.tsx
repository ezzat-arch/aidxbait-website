...
export default function CheckoutPage() {
	...
	const loadAddresses = useCallback(async () => {
		...
		try {
			...
			toast({
				...
				description: tToastDesc("failed_to_load_addresses"),
				variant: "destructive",
			});
		} finally {
	...
	const handlePlaceOrder = async () => {
		...
		if (!userProfile?.patient_id) {
			toast({
				title: tToastTitle("error"),
				description: tToastDesc("user_not_available"),
				variant: "destructive",
			});
			return;
		}
		...
		if (!selectedAddressId) {
			toast({
				title: tToastTitle("address_required"),
				description: tToastDesc("please_select_shipping_address"),
				variant: "destructive",
			});
			return;
		}
		...
		if (cart.items.length === 0) {
			toast({
				title: tToastTitle("empty_cart"),
				description: tToastDesc("your_cart_is_empty"),
				variant: "destructive",
			});
			return;
		}
		...
		try {
			...
			if (paymentMethod === "online") {
				...
				try {
					...
					toast({
						title: tToastTitle("payment_error"),
						description:
							paymentError instanceof Error
								? paymentError.message
								: tToastDesc("failed_to_initialize_payment"),
						variant: "destructive",
					});
				...
				toast({
					title: tToastTitle("order_placed"),
					description: tToastDesc("order_placed_successfully"),
				});
			...
			toast({
				...
				description:
					error instanceof Error
						? error.message
						: tToastDesc("failed_to_place_order"),
				variant: "destructive",
			});
			setIsSubmitting(false);
		}
		...
		<div className="min-h-screen bg-background pt-36">
			<div className="container mx-auto px-4 py-8">
				...
				<div className="flex items-center gap-4 mb-8">
					<Button variant="ghost" asChild className="ltr:-ml-4 rtl:-mr-4">
					...
					<div>
						...
						<p className="text-muted-foreground">{t("complete_your_order")}</p>
					</div>
				</div>
